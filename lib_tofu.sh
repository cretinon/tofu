...
# doc-verbatim: 1. **Description:** At library load time, when `MY_GIT_DIR` is set and `$MY_GIT_DIR/tofu/conf/tofu.conf` exists, that file is sourced and `TOFU_BIN` / `TOFU_DIR` / `TOFU_VAR_FILE` are exported -- but only when they are not already set in the environment (the environment always wins).
...
# description: Resolves the variable file to use: `$1` when given, `TOFU_VAR_FILE` otherwise, and prints it as an absolute path -- a relative path is looked up in the project directory first, exactly where `tofu -chdir` resolves it. An unset/empty value is accepted (no variable file).
# example: `_tofu_var_file "prod.tfvars"` — outputs the absolute path of the file, wherever it was found
# example: `_tofu_var_file` — outputs the absolute path of `terraform.tfvars` when that file exists in the project, otherwise nothing
...
_tofu_var_file () {
    ...
    if ! _exist "$__file"; then
        if _fileexist "$__dir/terraform.tfvars"; then __file="terraform.tfvars"; fi
    fi
...
# description: Shows the execution plan (`tofu plan -input=false`), optionally with a variable file and/or saving the plan to a file (keep it out of git: `*.tfplan` is ignored). `--dry-run` never writes the plan file.
...
# return: `10` (`ERROR_ARGV`) — variable file missing, or no usable binary/directory
# return: `1` — the `tofu plan` command failed.
_tofu_plan () {
    _func_start "$@"
    ...
    if ! __var_file=$(_tofu_var_file "${1:-}"); then _error "VAR_FILE: invalid variable file" ; _func_end "$ERROR_ARGV" ; return "$ERROR_ARGV" ; fi
    if _exist "$__var_file"; then __args+=("-var-file=$__var_file") ; fi
...
# description: Applies the configuration (`tofu apply -auto-approve`). Requires `--force` (or `TOFU_CONFIRM=true`); under `--dry-run` it only shows the plan.
...
# return: `1` — the `tofu apply` command failed.
_tofu_apply () {
    _func_start "$@"
...
# description: Destroys the managed guests (`tofu destroy -auto-approve`). Requires `--force` (or `TOFU_CONFIRM=true`); under `--dry-run` it only shows the destroy plan.
...
# return: `10` (`ERROR_ARGV`) — no `--force`, invalid variable file, or no usable binary/directory
# return: `1` — the `tofu destroy` command failed.
_tofu_destroy () {
    _func_start "$@"
...
_usage_tofu () {
    ...
    echo "  * Variable file    : --var-file (\$TOFU_VAR_FILE, else terraform.tfvars)"
